...
const Hero = () => {
  ...
  useEffect(() => {
    // Initialize animations only after all libraries are loaded
    if (!libsLoaded) return;

    // Ensure all libraries are available on the window object
    if (!window.gsap || !window.ScrollTrigger || !window.Lenis) {
      console.error("Animation libraries not loaded correctly.");
      return;
    }

    const { gsap, ScrollTrigger, Lenis } = window;

    // Use GSAP context for safe cleanup of animations
    const ctx = gsap.context(() => {
      ...
      // *** CRITICAL STEP ***
      // Sync Lenis scroll events with GSAP's ScrollTrigger
      lenis.on('scroll', ScrollTrigger.update);

      // Use GSAP's ticker to drive Lenis's animation frame for better synchronization
    ...
    }, containerRef); // Scope the context to the main container for easier cleanup

    return () => {
      ctx.revert(); // Revert all GSAP animations and kill ScrollTriggers within the context
      if (lenisRef.current) {
      ...
      <div ref={containerRef} className="min-h-screen bg-black text-white font-sans overflow-hidden">
        ...
        <section className="relative min-h-screen flex flex-col justify-center items-center px-6 md:px-12">
          ...
          <div className="text-center z-10">
            ...
            >
              DUO <span className='text-violet-600 font-yugi'>WEB</span>
            </h1>
        ...
        {/* Second Section */}
        <section ref={mainTextRef} className="section-animate min-h-screen flex flex-col justify-center items-center px-6 md:px-12 relative">
          <div className="text-center z-10">
